...
fruits = ["abacaxi", "uva", "maçã", "laranja", "banana", "melancia", "pera", "tomate"]
MAX_ATTEMPTS = 3
...
def guess_the_word():
    ...
    attempts.append(1)
    if len(attempts) < 3 and check_result(word, guesses):
        pass
    elif check_result(word, guesses) is False and len(attempts) < 3:
        guess_the_word()
    else:
